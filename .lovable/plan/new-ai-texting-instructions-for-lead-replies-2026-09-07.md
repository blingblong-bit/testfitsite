...
- Takeover: query `sms_conversation_log` for the newest outbound row with `from_ai = false`; if within the takeover window, keep the existing inbound insert and `last_response_at` / response-tracking updates intact (no early return before logging), then log a `system` row with `kind: "staff_takeover_suppressed"`, alert staff with the inbound text, and send no AI reply. Drip senders already respect `sequence_status = 'paused'`.

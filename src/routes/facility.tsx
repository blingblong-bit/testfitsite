    ...
    <>
      ...
      <section className="container-page py-16 md:py-20">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 text-muted-foreground leading-relaxed">
            Explore a quick look at the cardio area, strength floor, free weights, and training spaces that make FIT Beyond Plus clean, professional, and ready for every level.
          </p>
        </div>

        <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {galleryPhotos.map((photo) => (
            <figure key={photo.title} className="overflow-hidden rounded-lg border border-border bg-card">
      ...
      <section className="container-page pb-20 space-y-20">
        {sections.map((section, i) => (
          <div key={section.title} className={`grid items-center gap-10 md:grid-cols-2 ${i % 2 ? "md:[&>*:first-child]:order-2" : ""}`}>
            <img
              src={section.img}
              alt={section.alt}
              loading="lazy"
